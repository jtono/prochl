...
center_value = 27
d_temp <- d_temp %>% 
  mutate(CenteredTemp = scale(TempLevel, center = center_value , scale = FALSE))


require(nlme)

# Set paramters to correct form for mixed model fitting
d_temp$Rep <- as.factor(d_temp$Rep)
d_temp$Id <- as.factor(d_temp$Id)
d_temp$Stress <- as.factor(d_temp$Stress)
d_temp$TempLevel <- as.numeric(d_temp$TempLevel)
d_temp$CenteredTemp <- as.numeric(d_temp$CenteredTemp)

# group the data
df_grp <- groupedData(estimate ~ TempLevel | Id, data = d_temp)
df_grp <- groupedData(estimate ~ CenteredTemp | Id, data = d_temp)

# fit the model
gaus_nlme <- nlme(estimate ~ gaussian_1987(temp = TempLevel, rmax, topt, a),
                  data = df_grp,
                  ...
                  start = c(0.003, rep(0, 3), 27, rep(0, 3), 8, rep(0, 3)),
                  na.action = na.omit,
                  method = 'REML',
...
# fit the model
quad_temp_nlme <- nlme(estimate ~ quadratic_2008(temp = CenteredTemp, a, b, c),
                      data = df_grp,
                      fixed = list(a + b+ c ~ 1 + Stress),
                      random = pdDiag(a + b+ c ~ 1),
                      # start tells the model roughly where to start based on values in the data and the 3 represents the number of variables it will fit the model to not including the control (three other stressor combinations)
                      start = c(0.003, rep(0, 3),20, rep(0, 3), 30,rep(0, 3)),
                      na.action = na.omit,
                      method = 'REML',
                      control = nlmeControl(pnlsTol = 0.02))
summary(quad_temp_nlme)
...
quad_temp_nlme_2 <- update(quad_temp_nlme, random = pdDiag(a + b ~ 1))
quad_temp_nlme_3 <- update(quad_temp_nlme, random = pdDiag(a + c ~ 1)) 
quad_temp_nlme_4 <- update(quad_temp_nlme, random = pdDiag(b + c ~ 1))
quad_temp_nlme_5 <- update(quad_temp_nlme, random = pdDiag(a ~ 1))
quad_temp_nlme_6 <- update(quad_temp_nlme, random = pdDiag(b ~ 1))
#quad_temp_nlme_7 <- update(quad_temp_nlme, random = pdDiag(c ~ 1)) #failed


# Do ANOVA style analysis to get AIC and p values on which model fits best
anova(quad_temp_nlme, quad_temp_nlme_2, quad_temp_nlme_3, quad_temp_nlme_4, quad_temp_nlme_5, quad_temp_nlme_6) # full random effects model is best
...
quad_temp_nlme <- nlme(estimate ~ quadratic_2008(temp = CenteredTemp, a, b, c),
                       ...
                       random = pdDiag(a + b+ c ~ 1),
                       # start tells the model roughly where to start based on values in the data and the 3 represents the number of variables it will fit the model to not including the control (three other stressor combinations)
                       start = c(0.003, rep(0, 3),10, rep(0, 3), 20,rep(0, 3)),
                       na.action = na.omit,
                       method = 'ML',
                       control = nlmeControl(pnlsTol = 0.02))

# remove treatment effect on a (height)					  
quad_temp_nlme1<- update(quad_temp_nlme, fixed = list(a ~ 1, b + c ~ 1 + Stress), start = c(0.003, 10, rep(0, 3), 20, rep(0, 3)))

# remove treatment effect on b (position)				  
quad_temp_nlme2<- update(quad_temp_nlme, fixed = list(b ~ 1, a + c ~ 1 + Stress), start = c(0.003, rep(0, 3), 10, 20, rep(0, 3))) # failed

# remove treatment effect on c (breadth)				  
quad_temp_nlme3<- update(quad_temp_nlme, fixed = list(a + b ~ 1 + Stress, c ~ 1), start = c(0.003, rep(0, 3), 10, rep(0, 3), 20))


anova(quad_temp_nlme, quad_temp_nlme1, quad_temp_nlme2, quad_temp_nlme3) # the model without the fixed effect treatment for position has the lowest AIC values however this is not significantly different from the full model

# now remove more and see the effects and which is best model
# remove treatment effect on a (height)	 and b (position)				  
quad_temp_nlme4<- update(quad_temp_nlme, fixed = list(a + b ~ 1, c ~ 1 + Stress), start = c(0.003, 10, 20, rep(0, 3)))

# remove treatment effect on a (height) and c (breadth)			  
#quad_temp_nlme5<- update(quad_temp_nlme, fixed = list(b ~ 1 + Stress, a + b  ~ 1), start = c(0.003, 10, rep(0, 3), 20 )) # failed

# remove treatment effect on b (position) and c (breadth)				  
quad_temp_nlme6<- update(quad_temp_nlme3, fixed = list(a ~ 1 + Stress, b + c ~ 1 ), start = c(0.003, rep(0, 3), 10, 20))

# remove from model 2 height and position
quad_temp_nlme7<- update(quad_temp_nlme2, fixed = list(a + b ~ 1, c ~ 1 + Stress), start = c(0.003, 10, 20, rep(0, 3)))

# remove treatment effect on a (height) and c (breadth)			  
#quad_temp_nlme8<- update(quad_temp_nlme3, fixed = list(topt ~ 1 + Stress, rmax + a  ~ 1), start = c(0.003, 10, rep(0, 3), 20 ))

# remove treatment effect on position (b) and breadth (c)			  
quad_temp_nlme9<- update(quad_temp_nlme3, fixed = list(a ~ 1 + Stress, b + c ~ 1 ), start = c(0.003, rep(0, 3), 10, 20))

anova(quad_temp_nlme, quad_temp_nlme1,quad_temp_nlme2, quad_temp_nlme3,quad_temp_nlme4, quad_temp_nlme6, quad_temp_nlme7, quad_temp_nlme9) # 3rd model/ full model best here but not significantly different between

# model without position change is best (model 2) 
quad_temp_nlme_final <- update(quad_temp_nlme2, method = 'REML')

summary(quad_temp_nlme_final)
#### Gaussian Checks ####
# check the best model fit and check each combination of random effects (update updates the model above without having to write it all out)
gaus_nlme_2 <- update(gaus_nlme, random = pdDiag(rmax + topt ~ 1))
#gaus_nlme_3 <- update(gaus_nlme, random = pdDiag(rmax + a ~ 1)) # failed
gaus_nlme_4 <- update(gaus_nlme, random = pdDiag(a + topt ~ 1))
gaus_nlme_5 <- update(gaus_nlme, random = pdDiag(rmax ~ 1))
gaus_nlme_6 <- update(gaus_nlme, random = pdDiag(topt ~ 1))
#gaus_nlme_7 <- update(gaus_nlme, random = pdDiag(a ~ 1)) # failed


# Do ANOVA style analysis to get AIC and p values on which model fits best
anova(gaus_nlme, gaus_nlme_2, gaus_nlme_4, gaus_nlme_5, gaus_nlme_6) # full random effects model is best
...
gaus_nlme1<- update(gaus_nlme, fixed = list(rmax ~ 1, topt + a ~ 1 + Stress), start = c(0.003, 27, rep(0, 3), 8, rep(0, 3)))

# remove treatment effect on topt				  
gaus_nlme2<- update(gaus_nlme, fixed = list(topt ~ 1, rmax + a ~ 1 + Stress), start = c(0.003, rep(0, 3), 27, 8, rep(0, 3))) # failed

# remove treatment effect on breadth (a)				  
gaus_nlme3<- update(gaus_nlme, fixed = list(rmax + topt ~ 1 + Stress, a ~ 1), start = c(0.003, rep(0, 3), 27, rep(0, 3), 8))


anova(gaus_nlme, gaus_nlme1, gaus_nlme3) # the model without the fixed effet treatment for breadth (a) has the lowest AIC values however this is not significantly different from the full model
# need to ask Gab which model is best to use in this case? Would it be the one without breadth? 

# now remove more and see the effects and which is best model
# remove treatment effect on rmax	 and topt				  
gaus_nlme4<- update(gaus_nlme, fixed = list(rmax + topt ~ 1, a ~ 1 + Stress), start = c(0.003, 27, 8, rep(0, 3)))

# remove treatment effect on topt and a			  
gaus_nlme5<- update(gaus_nlme, fixed = list(topt ~ 1 + Stress, rmax + a  ~ 1), start = c(0.003, 27, rep(0, 3), 8 )) # failed

# remove treatment effect on breadth and topt				  
gaus_nlme6<- update(gaus_nlme3, fixed = list(rmax ~ 1 + Stress, topt + a ~ 1 ), start = c(0.003, rep(0, 3), 27, 8))

# remove from model 3
gaus_nlme7<- update(gaus_nlme3, fixed = list(rmax + topt ~ 1, a ~ 1 + Stress), start = c(0.003, 27, 8, rep(0, 3)))

# remove treatment effect on topt and a			  
gaus_nlme8<- update(gaus_nlme3, fixed = list(topt ~ 1 + Stress, rmax + a  ~ 1), start = c(0.003, 27, rep(0, 3), 8 )) # failed

# remove treatment effect on breadth and topt				  
gaus_nlme9<- update(gaus_nlme3, fixed = list(rmax ~ 1 + Stress, topt + a ~ 1 ), start = c(0.003, rep(0, 3), 27, 8))

anova(gaus_nlme, gaus_nlme1, gaus_nlme3,gaus_nlme4, gaus_nlme6, gaus_nlme7, gaus_nlme9) # 3rd model/ full model best here but not significantly different between

# full model best 
gaus_nlme_final <- update(gaus_nlme, method = 'REML')
